...
INITIAL_LANE_V2_RIGHT_BOX_SPEED = 0.23

# =====================================
# Lane v2 bbox following after timed right-bbox following
# =====================================
LANE_V2_CONF = 0.4
LANE_V2_ALLOWED_LABELS = {"round"} #right-left-lane
LANE_V2_RIGHT_BOX_CONF = 0.3
LANE_V2_MAX_BOXES = 2
LANE_V2_DIVERGE_DELTA_PX = 550.0
...
def main():
    ...
    print(
        ...
        f"stop {STOP_SIGN_STOP_SECONDS:.1f}s, "
        f"cooldown {STOP_SIGN_COOLDOWN_SECONDS:.1f}s"
    )
    ...
    try:
        while True:
            ...
            if (
                drive_mode == "ROAD_FOLLOW"
                and not avoid_mode
                and now >= stop_sign_cooldown_until
            ):
                (
                    stop_sign_detected,
                    stop_sign_width,
                ...
                if stop_sign_detected:
                    stop_sign_cooldown_until = (
                        now + STOP_SIGN_COOLDOWN_SECONDS
                    )
                    print(
                        f"[STOP SIGN] "
                        ...
                        f"then ignore stop signs until cooldown ends."
                    )
                    stop_vehicle()
            ...
            if drive_mode == "INITIAL_LANE_V2_RIGHT_BOX":
                ...
                if (
                    ...
                    print(
                        f"[INITIAL LANE_V2 DONE] "
                        f"{elapsed_initial_lane_v2:.2f}s elapsed. "
                ...
                if valid_lane:
                    ...
                    print(
                        ...
                        f"elapsed={elapsed_initial_lane_v2:.2f}/"
                        f"{INITIAL_LANE_V2_RIGHT_BOX_SECONDS:.2f}s "
                    ...
                    if mask_miss_count <= MAX_MASK_HOLD_FRAMES:
                        ...
                        print(
                            f"[INITIAL LANE_V2 MISS] hold prev steering "
                            f"{mask_miss_count}/{MAX_MASK_HOLD_FRAMES}"
                        )
                    else:
                        print("[INITIAL LANE_V2 LOST] stop for safety")
                        stop_vehicle()
                        time.sleep(0.03)
                        continue
            ...
            if drive_mode == "POST_FORCE_STOP":
                ...
                if elapsed_post_stop >= POST_FORCE_STOP_SECONDS:
                    print(
                        f"[POST INITIAL LANE_V2 STOP DONE] "
