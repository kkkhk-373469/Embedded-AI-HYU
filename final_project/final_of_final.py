...
lane_v2_model = (
    YOLO(LANE_V2_MODEL_PATH, task='detect')
...
# =====================================
# Dedicated stop sign detection
# =====================================
STOP_SIGN_CONF = 0.5
STOP_SIGN_MIN_WIDTH = 100
STOP_SIGN_STOP_SECONDS = 2.7
STOP_SIGN_LABELS = {"stop"}
...
# =====================================
# Timed lane_v3 right-bbox following after sign stop
# =====================================
SIGN_STOP_SECONDS = 0.5
INITIAL_LANE_V2_RIGHT_BOX_SECONDS = 2.4
POST_FORCE_STOP_SECONDS = 1.0
INITIAL_LANE_V2_RIGHT_BOX_SPEED = 0.21

# =====================================
# Lane v2 bbox following after timed right-bbox following
# =====================================
LANE_V2_CONF = 0.38
LANE_V2_ALLOWED_LABELS = {"right-left-lane"}
LANE_V2_RIGHT_BOX_CONF = 0.4
LANE_V2_MAX_BOXES = 2
LANE_V2_DIVERGE_DELTA_PX = 500.0
LANE_V2_TREND_EPS_PX = 5.0
LANE_V2_TREND_CONFIRM_FRAMES = 1
LANE_V2_COUNT_COOLDOWN_SECONDS = 0.7
...
# Lane/mask lost recovery. This is a timed 360-ish in-place scan.
SPIN_RECOVERY_SPEED = 0.15
SPIN_RECOVERY_SECONDS = 2.4
SPIN_RECOVERY_STEP_SECONDS = 0.03
SPIN_RECOVERY_DIRECTION = 1

# 예전에 쓰던 코드처럼 실제 전송 시 L/R을 반대로 보내야 하는 경우 True
SWAP_LR_COMMAND = True

# segmentation mask가 잠깐 안 잡혔을 때 이전 steering 유지
MAX_MASK_HOLD_FRAMES = 10
...
def spin_in_place_for_lane_recovery(reason, log=True):
    if log:
        print(
            f"[RECOVERY SPIN] {reason}: "
            f"spin in place for {SPIN_RECOVERY_SECONDS:.1f}s"
        )

    start_time = time.time()

    while time.time() - start_time < SPIN_RECOVERY_SECONDS:
        send_motor(
            -SPIN_RECOVERY_DIRECTION * SPIN_RECOVERY_SPEED,
            SPIN_RECOVERY_DIRECTION * SPIN_RECOVERY_SPEED
        )
        time.sleep(SPIN_RECOVERY_STEP_SECONDS)
...
def warmup_models():
    ...
    print("[WARMUP] stop.pt")
    stop_sign_model.predict(
        source=dummy_frame,
        imgsz=640,
        verbose=False
    )
...
def disable_tire_model():
    ...
    print("[TIRE] tire.engine disabled permanently")
...
def detect_stop_sign(frame, source):
    results = stop_sign_model.predict(
        source=frame,
        conf=STOP_SIGN_CONF,
        imgsz=640,
        verbose=False
    ...
    for result in results:
        ...
        for box in result.boxes:
            cls_id = int(box.cls[0].item())
            label = normalize_sign_label(stop_sign_model.names[cls_id])

            if label not in STOP_SIGN_LABELS:
                continue
    ...
    return max_width >= STOP_SIGN_MIN_WIDTH, max_width, max_conf, source


def detect_stop_sign_dual(left_frame, right_frame):
    detections = []

    if left_frame is not None:
        detections.append(detect_stop_sign(left_frame, "LEFT"))

    if right_frame is not None:
        detections.append(detect_stop_sign(right_frame, "RIGHT"))

    if not detections:
        return False, 0.0, 0.0, None

    return max(detections, key=lambda detection: detection[1])


def disable_stop_sign_model():
    global stop_sign_model

    stop_sign_model = None
    gc.collect()

    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    print("[STOP SIGN] best_final.pt disabled permanently")
...
def detect_sign_labels(frame):
    """
    sign.engine에서 conf 0.5 이상인 관심 라벨을 프레임 단위로 반환한다.
...
def get_top_lane_v2_boxes(frame):
    ...
    for r in results:
        ...
        for box in boxes:
            cls_id = int(box.cls[0].item())
            label_name = lane_v2_model.names.get(cls_id)

            if label_name is None:
                print(
                    f"[LANE_V2] ignore unknown class id={cls_id}, "
                    f"names={lane_v2_model.names}"
                )
                continue
...
def get_steering_from_lane_v2_rightmost_box(
    frame,
    camera_name,
    prev_steering
):
...
def get_steering_from_lane_v2_boxes(
    ...
    if len(lane_boxes) >= 2:
        ...
        first_diverge = diverge_count == 0
        now = time.monotonic()
        last_count_time = gap_state["last_count_time"]
        count_cooldown_ready = (
            last_count_time is None
            or now - last_count_time >= LANE_V2_COUNT_COOLDOWN_SECONDS
        )

        if (
            (first_diverge or gap_state["trend"] == "increasing")
            and current_pair_gap >= diverge_threshold
            and not gap_state["diverge_active"]
            and count_cooldown_ready
            and diverge_count < LANE_V2_RIGHT_BOX_COUNT
        ):
            diverge_count += 1
            gap_state["diverge_active"] = True
            gap_state["last_count_time"] = now
...
def update_vehicle_motion(steering, speed, log=True):
    """
...
def main():
    ...
    print(" Lane v2 right-bbox following before normal lane_v2 follow")
    print(f" Sign stop: circle width >= {CIRCLE_MIN_WIDTH}px")
    print(
        f" Stop sign: conf >= {STOP_SIGN_CONF}, "
        f"width >= {STOP_SIGN_MIN_WIDTH}px, "
        f"stop {STOP_SIGN_STOP_SECONDS:.1f}s, then disable model"
    )
    ...
    initial_lane_v2_start_time = None
    ...
    try:
        while True:
            ...
            if avoid_mode:
                ...
                t = time.time() - avoid_start

                if avoid_dir == 1:
                    shift_steer = 0.65
                    parallel_steer = -0.5
                    straight_steer = 0.0
                    return_steer = -0.6 #-0.6
                    restore_steer = 0.25
                else:
                    shift_steer = -0.75
                    parallel_steer = 0.2
                ...
                if t < 0.55:
                    ...
                    steering = shift_steer

                elif t < 1.35:

                    steering = parallel_steer


                elif t < 1.65:

                    steering = straight_steer

                elif t < 2.5:

                    steering = return_steer

                elif t < 2.9:

                    steering = restore_steer
            ...
            if (
                ...
                (
                    ...
                    stop_sign_conf,
                    stop_sign_source
                ) = detect_stop_sign_dual(
                    left_frame,
                    right_frame
                )

                if stop_sign_detected:
                    stop_sign_detection_enabled = False
                    disable_stop_sign_model()
                    print(
                        f"[STOP SIGN] "
                        f"camera={stop_sign_source} "
                        f"conf={stop_sign_conf:.2f} "
                        f"width={stop_sign_width:.1f}px. "
                        f"Stop for {STOP_SIGN_STOP_SECONDS:.1f}s, "
                        f"then keep stop-sign detection disabled."
                    )
                    stop_vehicle()
                    time.sleep(STOP_SIGN_STOP_SECONDS)
                    pid.reset()
                    prev_steering = 0.0
                    mask_miss_count = 0
                    print("[STOP SIGN] Resume ROAD_FOLLOW")
                    continue

            if tire_detection_enabled and drive_mode == "ROAD_FOLLOW":
                (
                    tire_detected,
                    cx,
                    x1,
            ...
            if (
                ...
                if source == "LEFT":
                    center = LEFT_CAMERA_CENTER
                else:
                    center = RIGHT_CAMERA_CENTER

                position = "LEFT" if cx < center else "RIGHT"
                ...
                if tire_same_count >= 3 and not avoid_mode:
            ...
            elif not tire_detected and not avoid_mode:

                tire_side=None
                tire_same_count=0

            in_roundabout_mode = drive_mode in {
                "INITIAL_LANE_V2_RIGHT_BOX",
                "POST_FORCE_STOP",
                "LANE_V2_FOLLOW"
            }

            if left_frame is None and right_frame is None:
                if not in_roundabout_mode:
                    print("[CAMERA] both frames missing, skip")
                continue

            if (
                not in_roundabout_mode
                and (left_frame is None or right_frame is None)
            ):
                print(
                    f"[CAMERA] "
            ...
            if drive_mode == "INITIAL_LANE_V2_RIGHT_BOX":
                elapsed_initial_lane_v2 = (
                    time.time() - initial_lane_v2_start_time
                )

                if (
                    elapsed_initial_lane_v2
                    >= INITIAL_LANE_V2_RIGHT_BOX_SECONDS
                ):
                    stop_vehicle()
                ...
                steering, valid_lane, debug = (
                    get_steering_from_lane_v2_rightmost_box(
                        right_frame,
                        "right",
                        prev_steering
                    )
                ...
                if valid_lane:
                    ...
                    if mask_miss_count <= MAX_MASK_HOLD_FRAMES:
                        ...
                        spin_in_place_for_lane_recovery(
                            "INITIAL_LANE_V2 lost",
                            log=False
                        )
                        pid.reset()
                        prev_steering = 0.0
                ...
                update_vehicle_motion(
                    steering,
                    INITIAL_LANE_V2_RIGHT_BOX_SPEED,
                    log=False
                )
                time.sleep(0.03)
                continue
            ...
            if drive_mode == "LANE_V2_FOLLOW":
                ...
                if valid_lane:
                    ...
                    bbox_distance = debug["pair_gap"]
                    distance_text = (
                        f"{bbox_distance:.1f}px"
                        if bbox_distance is not None
                        else "None"
                    )
                    print(
                        f"count={lane_diverge_count} "
                        f"bbox_distance={distance_text}"
                    )
            ...
            if circle_count >= CIRCLE_CONFIRM_FRAMES:
                ...
                print(
                    ...
                    f"Stop, then timed lane_v2 right-box follow."
                )
                stop_vehicle()
                ...
                prev_steering = 0.0
                mask_miss_count = 0
                drive_mode = "INITIAL_LANE_V2_RIGHT_BOX"
                initial_lane_v2_start_time = time.time()
                continue
            ...
            if valid_mask:
                ...
                if mask_miss_count <= MAX_MASK_HOLD_FRAMES:
                    ...
                    # 너무 오래 mask가 안 잡히면 제자리 회전으로 다시 차선을 찾는다.
                    spin_in_place_for_lane_recovery(
                        "segmentation mask lost"
                    )
                    pid.reset()
                    prev_steering = 0.0
                    mask_miss_count = 0
